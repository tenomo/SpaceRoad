...
public class SettingsExample : MonoBehaviour {
    ...
    void OnGUI() {
        ...
        GUI.Label(new Rect(10, 10, 300, 25), "Press '" + Controls.KeyCodeToString(SettingsAndControls.Controls.GetKeyCodeFor("Shoot"))
                        + "' to shoot and '" +
        ...
        if (GUI.Button(new Rect(430, 150, 300, 25), "MyTest")) {
    ...
    void TestMyChanges()
    {
        string name_note = "name_note";
        string value_note = "value_note";
        test testObj = new test();

         
        SettingsAndControls.Settings.SetSetting(name_note, testObj, Setting.SettingType.STRING);

        SettingsAndControls.Save();       

       // Debug.Log(SettingsAndControls.Settings.ge(name_note));

    }

    class test : SACObject
    {
